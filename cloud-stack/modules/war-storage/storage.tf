...
# Create third party network IDs to be given access to from the variable
locals {
  third_party_networks = [
    for d in var.upload_control.azure_third_party_networks :
    "/subscriptions/${d.subscription_id}/resourceGroups/${d.resource_group}/providers/Microsoft.Network/virtualNetworks/${d.network_name}/subnets/${d.subnet_name}"
  ]
}
...
resource "azurerm_storage_account" "war_storage" {
  ...
  network_rules {
    ...
    virtual_network_subnet_ids = concat(values(var.network_access_control.app_subnet_ids), local.third_party_networks)
    # Allow CI/CD or other trusted public IPs to reach the storage APIs
    ip_rules = var.network_access_control.public_ip_mask
  }

  # Files share is via Kerberos
